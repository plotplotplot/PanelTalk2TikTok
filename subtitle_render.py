...
def render_grouped_subtitle_overlay(
    ...
    def draw_word(x, y, word, color):
        if use_ttf:
            # Shadow
            if shadow_size > 0:
                draw.text((x + shadow_offset_x, y + shadow_offset_y), word, font=font_ttf, fill=(0, 0, 0))
    ...
    if subtitle_bg:
        ...
        if line_metas:
            ...
            bar_height = subtitle_bg_height if subtitle_bg_height and subtitle_bg_height > 0 else auto_height
    ...
    for line_words, line_active, y in lines:
        ...
        for i, word in enumerate(line_words):
            if not word and i != len(line_words) - 1:
                x += space_width
                continue
            color = (0, 255, 255) if i == line_active else (255, 255, 255)
